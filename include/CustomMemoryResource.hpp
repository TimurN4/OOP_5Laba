...
class CustomMemoryResource : public std::pmr::memory_resource
{
    ...
    protected :
        ...
        void* do_allocate(size_t bytes, size_t alignment) override
        {
            ...
            for(int i = 0; i < clean_blocks.size(); ++i) {
                if(clean_blocks[i].size >= bytes && clean_blocks[i].alignment == alignment) { // что если clean_blocks[i].size == bytes куда денется ост память
                //или она деаллоцирована и зачем alignment
        ...
        //     int index = -1;
        //     for(int i = 0; i < used_blocks.size(); ++i) {
        //         if(used_blocks[i].offset == static_cast<size_t*>(ptr) && used_blocks[i].size == bytes && used_blocks[i].alignment == alignment) {
        //             Block tmp{used_blocks[i].offset, used_blocks[i].size, used_blocks[i].alignment};
        //             clean_blocks.push_back(tmp);
        //         }
        //     }
        //     if(index != -1) {
        ...
        //     //throw std::logic_error("Удаление не выделяемой памяти\n");/////
        // }
